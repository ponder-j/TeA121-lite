...
export interface Project { id: string; name: string; file_count: number; updated_at: string; last_run_id?: string; description: string; debug_mode: boolean }
export interface ProjectCreate { name: string; description?: string | null; debug_mode?: boolean }
...
export interface Run { id: string; project_id: string; status: RunStatus; progress: number; duration_ms: number; created_at: string; completed_at?: string; analyzer_version: string; detector_id: string; detector_version: string; rule_pack_id: string; rule_pack_version: string; file_ids: string[]; mode: 'normal' | 'trace'; config?: Record<string, unknown>; cwe_scope: string[]; summary: { alarm_count: number; diagnostic_count: number; unsupported_count: number; error_count: number } }
